...
function renderMachineCard(machine) {
  ...
  `;
    ...
    div.addEventListener('drop', (e) => handleMachineDrop(e, div, machine));

    container.appendChild(div);
}

function handleMachineDrop(e, div, machine) {
    e.preventDefault();
    div.classList.remove('ring-2', 'ring-green-400');
    ...
    removeExistingWeatherBadge(div);

    const weather = getCurrentWeather();
    const { updatedTime, badgeHtml } = calculateWeatherAdjustment(weather, machine, potData);
    ...
    animateMachineMixing(div, machine, potData, updatedTime);
}

function calculateWeatherAdjustment(weather, machine, potData) {
    let mixTimes = Array.isArray(potData.contents)
    ...
    if (weather) {
        ...
        if (temp > 35 && activeMixingMachines.size >= 1) {
            ...
            return { updatedTime: null, badgeHtml: null };
        }
    }

    return {
        updatedTime: Math.round(longestTime),
        badgeHtml: extraText ? `${icon}<div>${extraText.trim()}</div>` : null
    };
}

function removeExistingWeatherBadge(container) {
    const oldBadge = container.querySelector('.weather-badge');
    if (oldBadge) oldBadge.remove();
}

function animateMachineMixing(div, machine, potData, duration) {
